...
describe("GET to api/v1/status", () => {
  describe("Anonymus user", () => {
    describe("Check database status and format", () => {
      test("For check format update_at", async () => {
      ...
      test("For check database information", async () => {
        const response = await fetch("http://localhost:3000/api/v1/status");
        expect(response.status).toBe(200);
        ...
        expect(
          parseInt(responseBody.depedencies.database.max_connections),
        ).toEqual(100);
